        ...
        struct cts_device_touch_info *touch_info)
{
    ...
    int i;
    int contact = 0;
	int palm_major, palm_minor, palm_flags;
#ifdef CONFIG_CTS_SLOTPROTOCOL
	...
	palm_major = touch_info->palm_major & 0xFE;
	palm_minor = touch_info->palm_minor & 0xFE;
	palm_flags = touch_info->palm_flags & 0x01;
    for (i = 0; i < num; i++) {
        u16 x, y;
        ...
        cts_dbg("  Process touch msg[%d]: id[%u] ev=%u x=%u y=%u p=%u maj=%u min=%u palm=%u",
            ...
            palm_major, palm_minor, palm_flags);
#else
...
#ifdef SEC_PALM_FUNC
            input_report_key(input_dev, BTN_PALM, palm_flags);
            input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR, palm_major);
            input_report_abs(input_dev, ABS_MT_TOUCH_MINOR, palm_minor);
#else
