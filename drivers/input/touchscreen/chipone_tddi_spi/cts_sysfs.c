...
static ssize_t cts_high_sensitivity_mode_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    ...
    cts_lock_device(cts_dev);
    ret = cts_tcs_get_high_sensitivity_mode(cts_dev, &enabled);
    cts_unlock_device(cts_dev);
    if (ret) {
        cts_err("get high sensitivity mode failed!");
        return scnprintf(buf, PAGE_SIZE, "Read error\n");
    }

    return scnprintf(buf, PAGE_SIZE, "high sensitivity mode: 0x%02x\n", enabled);
}

static ssize_t cts_high_sensitivity_mode_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    ...
    if (enable == -1) {
        cts_err("Invalid arg for high sensitivity mode enable");
        return -EINVAL;
    }
...
static DEVICE_ATTR(high_sensitivity_mode,  S_IWUSR | S_IRUGO,
            cts_high_sensitivity_mode_show, cts_high_sensitivity_mode_store);
...
static struct attribute *cts_dev_misc_atts[] = {
    ...
    &dev_attr_high_sensitivity_mode.attr,
#ifdef CONFIG_CTS_TP_PROXIMITY
